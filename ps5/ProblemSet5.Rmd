...
#linearSVM_Mod <- ksvm(class ~ X1+X2 , data = svm_train, type = "C-svc", 
                       # kernel = "vanilladot",C = 10000, cross = 5, 
                      #  prob.model = TRUE)
#cross(linearSVM_Mod) #0.004137931
#
...
```
From the result table, we can see that the training dataset is perfectly separable, since as we set Cost to be extremely high, the misclassification rate in the training set is zero. Our linear model perfectly separates the classes. However, while we see a relatively low expected rate of misclassification, the actual misclassification rate was much higher than expected. Data coming from the same DGF can still yield training errors, because SVM does not rely on any assumptions of functional forms, and thus coming from the same function does not really help. (????) In addition, the zero misclassification rate on training set shows us that there may be overfitting happening, where the SVM picks up noise as signals, while noise in training data are not transferrable to test data.
