...
ggplot(data=cf, aes(x=cf1, y=cf2)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        axis.title.y = element_text(face="bold", colour="red", size = 12))
```
The value of lambda that minimizes the expected prediction error is 0.74 according to the 10 fold cross validation. I stick with the suggested lambda value for the model, because based on the plot, expected prediction error picks up pretty obviously as lambda increases.
...
ggplot(data=cf, aes(x=cf1, y=cf2)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        axis.title.y = element_text(face="bold", colour="red", size = 12))
```
The value of lambda that minimizes the expected prediction error is 0.03343 according to the 10 fold cross validation. I stick with the suggested lambda value for the model, because based on the plot, expected prediction error picks up pretty obviously as lambda increases.
...
p4p1_df <- data.frame(t(data.frame(lm_pred_metrics(p4p1_model_1), lm_pred_metrics(p4p1_model_2), lm_pred_metrics(p4p1_model_3), lm_pred_metrics(p4p1_model_4), lm_pred_metrics(p4p1_model_5), lm_pred_metrics(p4p1_model_6), lm_pred_metrics(p4p1_model_7), lm_pred_metrics(p4p1_model_8), lm_pred_metrics(p4p1_model_9), lm_pred_metrics(p4p1_model_10))))
num <- c(1:10)
p4p1_df <- data.frame(num, p4p1_df)
ggplot(p4p1_df, aes(x=num, y=LOOCV.1)) + geom_point() + xlab("degree polynomial")
```
...
optimalModel<-p4p1_model_3 <- lm(c_train_q4$Outstate~poly(c_train_q4$S.F.Ratio,3))
predResult<-data.frame(S.F.Ratio=c_train_q4$S.F.Ratio,predOutState=predict(optimalModel, newdata = c_train_q4))
predResultPlot<-qplot(x=S.F.Ratio,y=predOutState,data=predResult,geom=c("point"),ylab="out of state tuition prediction",xlab="student/faculty ratio")
predResultPlot
```
...
#via forLoop AIC check
bestDf<-0
AIC<-100000
for(i in 5:20){
  cubicSpline<-lm(Outstate~ns(S.F.Ratio,df=i,degree=2),data=c_train_q4)
  tempCheck<-lm_pred_metrics(cubicSpline)[1]
  #print(tempCheck)
  #print(i)
  if(tempCheck<AIC){
    bestDf<-i
  ...
  #testGraph: recommend starts w 5
  #cubicSplinePredResult<-data.frame(S.F.Ratio=c_train_q4$S.F.Ratio,predOutState=predict(cubicSpline, newdata = c_train_q4))
  #cubicSplinePredResultPlot<-qplot(x=S.F.Ratio,y=predOutState,data=cubicSplinePredResult,geom=c("point","smooth"),ylab="out of state tuition prediction",xlab="student/faculty ratio", main = toString(i))
  #print(cubicSplinePredResultPlot)
}
print(bestDf)
```
...
```{r}
#natural spline(cubic)
#via forLoop graph check
for(i in 1:20){
  cubicSpline<-lm(Outstate~ns(S.F.Ratio,df=i,degree=2),data=c_train_q4)
  #testGraph: recommend starts w 5
  cubicSplinePredResult<-data.frame(S.F.Ratio=c_train_q4$S.F.Ratio,predOutState=predict(cubicSpline, newdata = c_train_q4),Outstate=c_train_q4$Outstate)
  predResultPlot<-
    ggplot(cubicSplinePredResult,aes(x=S.F.Ratio,y=predOutState),color='red')+
    geom_point()+
    geom_point(data=cubicSplinePredResult,aes(y=OutState),color='black')
  print(predResultPlot)
}
```
...
```{r}
#smoothing spline
#compare and find the min GCV in range lambda=0:20
bestLambda<-0
smoothGcv<-10000
tempNum<-seq(0,20,0.01)
for(i in tempNum){
  smoothSplineModel<-smooth.spline(c_train_q4$S.F.Ratio,c_train_q4$Outstate,lambda=i)
  temp<-smoothSplineModel$cv.crit
  print(temp)
  if (smoothGcv>temp){
    smoothGcv<-temp
    bestLambda<-i
...
print(bestLambda)
# optimal model we find with smoothing spline
optSmoothSplineModel<-smooth.spline(c_train_q4$S.F.Ratio,c_train_q4$Outstate,lambda=bestLambda)
smoothSplinePredResult<-data.frame()
```
