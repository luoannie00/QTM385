...
Yes, the same result holds in the heldout test data, where the optimal ridge and lasso models perform better than full and subset OLS models. The ridge and lasso models improve EPE for the problem by selecting important variables and ignoring the rest of the noise variables. In that way, model variance is reduced since the ridge and lasso models are less likely to overfit to the training data, therefore improving EPE.
...
```{r}
#if we want to include one interaction term with tensor interaction ti()
tiMinGcv<-100
#summary()
for (i in 3:6){
  for (j in 3:6){
...
```{r}
# so we have four optimal models with/without interaction in various formula:
