...
```{r p4p1, warning=FALSE}
#initial recoding: use c_train_q4 moving on for consistence
c_train_q4 <- c_train
c_train_q4$Outstate <- log(c_train_q4$Outstate)

approachPlot <- qplot(x=S.F.Ratio, y=Outstate, data=c_train_q4, geom=c("point","smooth"), 
                      ylab="out of state tuition", xlab="student/faculty ratio")
approachPlot
...
lmApprochPlot <- qplot(x=S.F.Ratio, y=Outstate, data=c_train_q4, geom=c("point","smooth"), 
                       method="lm", ylab="out of state tuition", xlab="student/faculty ratio")
lmApprochPlot
```
...
optimalModel <- lm(c_train_q4$Outstate ~ poly(c_train_q4$S.F.Ratio,3))
predResult <- data.frame(S.F.Ratio=c_train_q4$S.F.Ratio,
                         predOutState=optimalModel$fitted.values,
                         originalOutState=c_train_q4$Outstate)

predResultPlot <- ggplot(data=predResult) +
