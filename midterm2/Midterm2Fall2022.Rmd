...
### Part 1 Solution
From the observation of shoe pictures, we found that a commonality is that the 12th column has dark marks (most of them have all 12 rows filled at 12th column). The 3s, 5s, and 8s, however, are not likely to have the whole or most of 12th column filled. So we picked three points at the 12th column after transforming all observations to 12x12 matrices and tune a grayscale value to find as many shoes as possible for the first step.
```{r}
shoes <- c()
for (i in 1:7500){
...
```
Now that we have found 18 shoes, we then matched those pictures with the ones given on the problem set, and we figured out the 2 shoes that are to be found. It makes sense that our previous method failed to identify those two since their 12th column, and more importantly, the middle part of the 12th column, was not oencil-marked. Again, we studied the characteristics of the 2 shoe pictures that are most different from number digits. For one shoe, both its upper-right corner and lower-left corner seem to be marked, which should be rare for digits. So let's try this condition.
```{r}
obs <- c()
for (i in 1:7500){
  p4_trainMatrix <- matrix(nrow = 12, ncol = 12, p4_trainX[i,], byrow = TRUE)
...
obs
```
After plotting, we see that 1244th obs is the shoe we wanted to find. We append this shoe to our shoes list.
```{r}
shoes <- append(1244, shoes)
```
Now we only have one shoe left to find. We observe that its middle-left and two points on the right are marked. We reasoned that those three points may be more likely to be filled for digits too, so we start by setting a high value for grey-scale value.
```{r}
obs <- c()
for (i in 1:7500){
  p4_trainMatrix <- matrix(nrow = 12, ncol = 12, p4_trainX[i,], byrow = TRUE)
...
obs
```
Looking at the pictures, we see that the second plot is the final shoe we wanted, which is the 2190th obs.
```{r}
