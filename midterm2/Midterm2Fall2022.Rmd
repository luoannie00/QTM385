...
### Part 3 Solution
We have used Logistic Regression, QDA, and NB w/ KDE for continuous margins. As explained above, we will not use NB without KDE due to high dimensionality. Since there are only two classes--four and nine, we won't be using Multinomial logistic regression. We will first continue our prediction with the lasso model we built before. Then, we will go back to NB and treat four and nine as discrete variables. Finally, we will build new models using LDA, generalized additive LR, Bayes' Theorem, 
```{r}
#lasso
