...
```
解释还没写
Class for label 6 and 8 seem to show the most within class variation over images; class for label 1 and 0 seem to show the least within class variation over images.
...
#We believe that RF is likely to perform better than Bagging, and it's computationally expensive to grow trees. After conventionally picking a value of num.tree=1000 for bagging, we found that the oos EPE did not decrease significantly. So instead of tuning for num.trees, we decided to move onto RF.
...
```
We can see that polySVM performed the best. 解释没写。比ada和xgb都好一大些lol
...
We will be using MLR, LASSO, LDA, QDA, NB w/ continuous variable w/ KDE, 3 SVM methods, and tree methods.
```{r MLR}
#Factored Data
...
for(i in 1:length(cost_vals)){
  #Train the SVM
  svm1 <- ksvm(train_yFactored ~ . , data = train_factored, type = "C-svc", kernel = "vanilladot", 
                C = cost_vals[i], cross = 5, prob.model = TRUE, kpar = list())
  #Extract the 5fold CV est and store
  temp <- kernlab::cross(svm1)
  if (temp<optimal_epe){
    optimal_epe<-temp
    optimal_cost<-cost_vals[i]
  }
...
# finalized model
linearSVM_classfier <- ksvm(train_yFactored ~ . , data = train_factored, type = "C-svc", 
                               kernel = "vanilladot", C = optimal_cost, cross = 5, prob.model = TRUE, 
                               kpar = list())
linearSVM <- predict(linearSVM_classfier, newdata = valid_factored , type = "response")
linearSVM_oos<- mean(linearSVM != valid_factored$train_yFactored)
result$linearSVM <- linearSVM_oos
...
for (j in 2:3){ #conventionally 2nd or 3rd degree polynomial would be enough
  for(i in 1:length(cost_vals)){
    #Train the SVM
    svm2 <- ksvm(train_yFactored ~ . , data = train_factored, type = "C-svc", kernel = "polydot", 
                 kpar = list(degree = j), C = cost_vals[i], cross = 5, prob.model = TRUE)
    #Extract the 5fold CV est and store
...
# finalized model
polySVM_classfier <- ksvm(train_yFactored ~ . , data = train_factored, type = "C-svc", 
                             kernel = "polydot", kpar = list(degree = optimal_degree), C = optimal_cost,
                             cross = 5, prob.model = TRUE)
...
for(i in 1:length(cost_vals)){
  #Train the SVM
  svm3 <- ksvm(train_yFactored ~ . , data = train_factored, type = "C-svc", kernel = "rbfdot", 
                C = cost_vals[i], cross = 5, prob.model = TRUE)
  #Extract the 5fold CV est and store
...
# finalized model
radialSVM_classfier <- ksvm(train_yFactored ~ . , data = train_factored, type = "C-svc", 
                            kernel = "rbfdot", C = optimal_cost, cross = 5, prob.model = TRUE)
