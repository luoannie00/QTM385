...
4&9(sample49) and 7&9(sample79) are the most often misclassified pairs
The following are two example we pull out from the code above.

potential aspects of misclassification:
For 7-9 misclassification, the hand-written 7 is a bit too curve-in on its left end.
For 4-9 misclassification, the location of the "circle" is a bit confusing for the model.[maybe the model consider circle part on the right side as a sign of 4-ness]

potential steps to improve in data cleaning stage:
1) improve on the digit-orientation algorithm
2) don't reduce the pixel to 12*12 and sacrifice some run-time for the quality
