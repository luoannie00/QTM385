...
```{r}
#According to problem 1 part 2, we can choose a subset of important variables to fit a GAM model, since the full predictor GAM model will lead to multicollinearity problem. Similarly, the full OLS linear regression model has the same problem. However, since the GAM model always has better performance over OLS linear regression model in terms of predictive accuracy, we decide not to try it.
...
```{r}
#The full predictor KNN model does not perform well here, which is probably caused by the curse of dimensionality problem.
knn <- seq(5,1000,100)
knn_mse_out <- c()
...
```{r}
p2_m <- c('random forest','GAM','LASSO','RIDGE','KNN')
#manually calculated
p2_runtime_seconds <- c(198,11,8,5,27)
data.frame(p2_m,p2_runtime_seconds)
```

As shown in the table, the runtime in random forest (our optimal model) is the highest even though it is the best predictive model. KNN model ranks the second highest, whereas GAM, LASSO, and RIDGE models are relatively quick.

```{r}
p2_results <- data.frame(ID = test_df$ID, 
                         p2_pred = predict(p2_rf_m, data=test_df)$predictions)
write.csv(p2_results, file='Midterm1Q2_Predictions.csv', row.names=FALSE)
```
...
```{r}
#final train with huge training set
set.seed(77)
sam_train <- as.numeric(sample(x=nrow(train_df), size=30000, replace=FALSE))
strain_df_p4 <- train_df[sam_train,]
final_train_df_4RFvar<-data.frame(shares=strain_df_p4$shares,kw_avg_avg=strain_df_p4$kw_avg_avg,self_reference_avg_sharess=strain_df_p4$self_reference_avg_sharess,kw_max_avg=strain_df_p4$kw_max_avg,n_unique_tokens=strain_df_p4$n_unique_tokens)
final_train_df_4RFvar$shares <- log(final_train_df_4RFvar$shares)
```


```{r}
bestModel<-ranger(data=final_train_df_4RFvar, formula = shares~., mtry=4, num.trees=1000)
...
# save the .csv
write.csv(outPutFile,"Midterm1Q4_Predictions.csv", row.names = FALSE)
```
...
#most ideal variable is kw_avg_avg based on out-of-sample EPE, which is 0.8152503

#in-sample using KCV
p6_knn_inMSE <- c()
for (j in 1:59) {
  knn_model_k <- knn.reg(train=as.data.frame(strain_df_p6[,j]),
                         test=as.data.frame(strain_df_p6[,j]),
                         y=as.data.frame(strain_df_p6[,60]),
                         k=optimalk[j])
  p6_knn_inMSE <- c(p6_knn_inMSE,mse(knn_model_k$pred,
                                     strain_df_p6$shares))
}
p6_knn_in_df <- data.frame(var = colnames(strain_df_p6[,-60]), p6_knn_inMSE)
p6_knn_in_df %>% arrange(p6_knn_in_df$p6_knn_inMSE)
```
For KNN, we found that "kw_avg_avg" is a good uni-predictor for both out-of-sample and in-sample EPE. Therefore, we will go with "kw_avg_avg" for our KNN model.
```{r}
...
### LASSO
```{r}
p6_x <- data.matrix(strain_df_p6[,-60])
...
### Conclusion and Plot
Based on p6_result table, we can see that all methods produced "kw_avg_avg" as the most important variable for univariable predictions, so we will choose this variable. Random Forest overall performs worse than others. Polynomial expansion, Spline (GAM), and KNN have more or less the same mse, and we decide to go with Spline (GAM) and we use "kw_avg_avg." 
```{r}
