...
for (i in mtry) {
  ...
  mse <- c(mse,mse(stest_df$shares, p1p1_pred$predictions))
}
```

```{r}
p1p1_df <- data.frame(mtry,oob,mse)

ggplot(data=p1p1_df) +
  geom_point(aes(x=mtry, y=mse)) +
  geom_line(aes(x=mtry, y=mse)) +
  theme_bw()
...
The number of features considered for each bootstrap tree that yields the lowest OOB error as well as the lowest error on the heldout test set is 8.
