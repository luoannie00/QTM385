...
#question - do we need to set seed for every chunk of code???
set.seed(123)
strain_df <- train_df[sample(x=nrow(train_df), size=7500, replace=FALSE),]
stest_df <- train_df[sample(x=nrow(train_df), size=2500, replace=FALSE),]

#transform variable shares
strain_df$shares <- log(strain_df$shares)
stest_df$shares <- log(stest_df$shares)

#drop variable url
strain_df <- strain_df[,-1]
stest_df <- stest_df[,-1]
```
...
for (i in mtry) {
  p1p1_m <- ranger(data=strain_df, formula = shares~., mtry=i, num.trees=1000)
  
  oob <- c(oob,p1p1_m$prediction.error)
  p1p1_pred <- predict(p1p1_m, data=stest_df)
  rf_mse <- c(rf_mse,mse(stest_df$shares, p1p1_pred$predictions))
}
```
...
The number of features considered for each bootstrap tree that yields the lowest OOB error is 8, whereas the number of features that yields the lowest error on the heldout test set is 20.
...
```{r}
p1p2_m <- ranger(data=strain_df, formula = shares~., mtry=8, num.trees=1000, 
...
```{r}
p1p2_df %>% arrange(desc(var_imp)) %>% head(2)

#true relationships
ggplot(data=strain_df) +
...
pred_r <- data.frame(kw_avg_avg = strain_df$kw_avg_avg, 
                     kw_max_avg = strain_df$kw_max_avg,
...
```{r}
#question - url???
#warning comes from the collinearity problem
#question - how to solve???
simple_lm <- lm(data=strain_df, formula = shares ~ .)
#in sample mse
mse(unname(predict(simple_lm, newdata=strain_df)), strain_df$shares)
#out of sample mse
mse(unname(predict(simple_lm, newdata=stest_df)), stest_df$shares)
```

```{r}
p2_x <- data.matrix(strain_df[,-60])
p2_y <- data.matrix(strain_df[,60])
...
#out of sample mse
mse(stest_df$shares, predict(p2_optimal_lassom, newx=data.matrix(stest_df[,-60])))
```

```{r}
knn <- c(5,10,15,20,25,30,35,40,45,50)
knn_mse_out <- c()

for (i in knn) {
  knn_m <- knn.reg(train=p2_x, test=stest_df[,-60], y=p2_y, k=i)
  knn_mse_out <- c(knn_mse_out,mse(knn_m$pred, stest_df$shares))
}
