...
low1 <- loess(shares ~ strain_df_p6[,27], data = strain_df_p6, span = .55)
low2 <- loess(shares ~ strain_df_p6[,27], data = strain_df_p6, span = .75)
low3 <- loess(shares ~ strain_df_p6[,27], data = strain_df_p6, span = .95)
low_nb_df <- data.frame(mse(strain_df_p6$shares, predict(low1)), mse(strain_df_p6$shares, predict(low2)), mse(strain_df_p6$shares, predict(low3)))
#test for binary variable
p6_LOESS <- c()
p6_LOESS_index <- c()
low1 <- loess(shares ~ strain_df_p6[,33], data = strain_df_p6, span = .55)
low2 <- loess(shares ~ strain_df_p6[,33], data = strain_df_p6, span = .75)
low3 <- loess(shares ~ strain_df_p6[,33], data = strain_df_p6, span = .95)
low_b_df <- data.frame(mse(strain_df_p6$shares, predict(low1)), mse(strain_df_p6$shares, predict(low2)), mse(strain_df_p6$shares, predict(low3)))
