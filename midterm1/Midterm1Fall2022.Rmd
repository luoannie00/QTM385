...
p6_knn_weekday_is_wednesday_mse <- data.frame(var = p6_knn_out_df[33,1], mse_out = p6_knn_out_df[33,2], five_fold = p6_knn_in_df[33,2], ten_fold = p6_knn_in_df[33,3])
p6_knn_weekday_is_wednesday_mse
```
We recorded the EPE of KNN model using weekday_is_wednesday above. We can see that the in-sample EPE is surprisingly good, but since its oos EPE doesn't look ideal, we will wait and see other models.
...
p6_poly_in <- c()
#exclude binary variables and variables with small number of possible values since degree of polynomial cannot exceed possible values
...
test <- lm_pred_metrics(p6_poly_1)
p6_poly_df <- t(cbind(lm_pred_metrics(p6_poly_1), lm_pred_metrics(p6_poly_2)))
...
ggplot(data=p4p1_df) + 
  ...
  theme_bw()
# We use LOOCV as our measure of EPE and find that third order of global polynomial that minimizes EPE. From the graph above, we could see that the LOOCV reaches minimum for degree polynomial=3.
```
```
